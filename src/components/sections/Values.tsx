      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 border-t border-white/15">
        {CORE_VALUES.map((v, i) => (
          <Reveal
            key={v.k}
            delay={(i % 3) * 0.08}
            className="group border-b border-r border-white/15 p-8 md:p-10 hover:bg-[#fd0000] transition-colors duration-500 relative"
          ...
          {WHY.map((w, i) => (
            <Reveal key={w.title} delay={(i % 3) * 0.08} className="py-8 border-t border-white/15 pr-6">
              <div className="flex items-start gap-4">
                <span className="font-mono text-[#fd0000] text-sm mt-1">{String(i + 1).padStart(2, "0")}</span>
                <div>
