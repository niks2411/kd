  ...
  <section id="products" className="py-28 md:py-40 mx-auto max-w-[1600px] px-6 md:px-12 bg-[#ffffff]" data-testid="products">
    <div className="grid lg:grid-cols-12 gap-8 items-end mb-16 md:mb-20">
      ...
      <div className="lg:col-span-4">
        <Reveal delay={0.2}>
          ...
          <Link href="/products" className="inline-flex items-center gap-1.5 text-xs font-mono font-semibold uppercase tracking-wider text-[#fd0000] hover:underline">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-6 gap-px bg-[#E5E7EB] border border-[#E5E7EB]">
      {PRODUCTS.slice(0, 4).map((p, i) => (
        <Reveal
          key={p.id}
          delay={(i % 2) * 0.08}
          className={`group relative bg-[#ffffff] overflow-hidden ${i % 3 === 0 ? "lg:col-span-4" : "lg:col-span-2"}`}
    ...
    <Reveal delay={0.1} className="mt-12 flex flex-wrap items-center gap-3">
      ...
      {OTHER_CAPABILITIES.map((c) => (
        <span key={c} className="border border-[#E5E7EB] bg-white px-4 py-2 text-sm font-medium text-[#0A0A0A] hover:border-[#fd0000] hover:text-[#fd0000] transition-colors duration-300">
          {c}
        </span>
      ))}
    </Reveal>
