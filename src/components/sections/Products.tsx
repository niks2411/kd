  ...
  <section id="products" className="py-28 md:py-40 mx-auto max-w-[1600px] px-6 md:px-12 bg-[#F9FAFB]" data-testid="products">
    <div className="grid lg:grid-cols-12 gap-8 items-end mb-16 md:mb-20">
      <div className="lg:col-span-8">
        <Reveal><Overline color="text-[#002FA7]">Our Product Portfolio</Overline></Reveal>
        <Reveal delay={0.1}>
      ...
      <div className="lg:col-span-4">
        <Reveal delay={0.2}>
          ...
          <Link href="/products" className="inline-flex items-center gap-1.5 text-xs font-mono font-semibold uppercase tracking-wider text-[#002FA7] hover:underline">
        ...
        >
          <Link href="/products" className="block relative h-full min-h-[300px] flex flex-col justify-end p-8 md:p-10">
            ...
            <div className="relative z-10">
              <span className="font-mono text-xs uppercase tracking-[0.2em] text-[#002FA7] font-semibold">{p.tag}</span>
              <h3 className="font-display text-2xl md:text-3xl font-medium mt-3 tracking-tight flex items-center gap-2 text-[#0A0A0A]">
                {p.title}
                <ArrowUpRight size={20} className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-[#002FA7]" />
              </h3>
    ...
    <Reveal delay={0.1} className="mt-12 flex flex-wrap items-center gap-3">
      ...
      {OTHER_CAPABILITIES.map((c) => (
        <span key={c} className="border border-[#E5E7EB] bg-white px-4 py-2 text-sm font-medium text-[#0A0A0A] hover:border-[#002FA7] hover:text-[#002FA7] transition-colors duration-300">
          {c}
        </span>
      ))}
    </Reveal>
